...
function setRowHeight(){
  const $rankTable = $('#ranktable'),
        $allDataRows= $rankTable.find('tbody > tr'),
        totalNumberOfDataRows = $allDataRows.length;

  $allDataRows.each((indexOfEachRow, oneTableRow)=>{
    let $totalRankScoreColumn = $(oneTableRow).find('.js-rankTotl'),
        totalRankScoreOfEachStudent = $totalRankScoreColumn.text(),
        desiredRowHeight = totalRankScoreOfEachStudent - (indexOfEachRow + 1);
    $(oneTableRow).find('td').css('line-height',desiredRowHeight);
  });
}
